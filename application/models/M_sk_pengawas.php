<?php
class M_sk_pengawas extends CI_Model{
	...
	function upload($id_ujian, $berkasbaru){
		$hasil=$this->db->query("UPDATE tb_ujian SET file_sk_pengawas='$berkasbaru' WHERE id_ujian='$id_ujian';");
		return $hasil;
	}

	function hapus_berkas($id_ujian){
		$hasil=$this->db->query("UPDATE tb_ujian SET file_sk_pengawas='' WHERE id_ujian='$id_ujian';");
		return $hasil;
	}
