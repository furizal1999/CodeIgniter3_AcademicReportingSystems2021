<?php
class M_nilai_hasil_ujian extends CI_Model{
	...
	function cekKetersediaanfileBerkasSoalUjian($id_ujian, $id_jadwal_kelas_pertemuan){
	    $hasil=$this->db->query("SELECT * from tb_berkas_ujian_kelas WHERE nama_berkas='Nilai Hasil Ujian' AND id_ujian='$id_ujian' AND id_jadwal_kelas_pertemuan='$id_jadwal_kelas_pertemuan' AND status='Tersedia'");
	    if($hasil->num_rows() > 0){
	...
	function upload($id_ujian,$id_jadwal_kelas_pertemuan, $berkasbaru){
		...
		$hasil=$this->db->query("INSERT INTO tb_berkas_ujian_kelas (id_jadwal_kelas_pertemuan, id_ujian, nama_berkas, nama_file_berkas, waktu_input_berkas, nama_file_berkas_valid, waktu_input_berkas_valid, status) VALUES ($id_jadwal_kelas_pertemuan, $id_ujian, 'Nilai Hasil Ujian', '$berkasbaru', '$sekarang','','0000-00-00 00:00:00', 'Tersedia')");
		return $hasil;
	}
	...
	// 	date_default_timezone_set('Asia/Jakarta');
 //        $sekarang = date("Y-m-d H:i:s");
	// 	$hasil=$this->db->query("UPDATE tb_berkas_ujian_kelas SET nama_file_berkas_valid='$berkasbaru', waktu_input_berkas_valid='$sekarang' WHERE id_berkas_ujian_kelas='$id_berkas_ujian_kelas' AND nama_berkas='Nilai Hasil Ujian';");
	// 	return $hasil;
	// }

	function hapus_berkas($id_berkas_ujian_kelas){
		$hasil=$this->db->query("UPDATE tb_berkas_ujian_kelas SET status='Dihapus' WHERE id_berkas_ujian_kelas='$id_berkas_ujian_kelas' AND nama_berkas='Nilai Hasil Ujian';");
		return $hasil;
	}

	// function hapus_berkas_prodi($id_berkas_ujian_kelas){
	// 	$hasil=$this->db->query("UPDATE tb_berkas_ujian_kelas SET nama_file_berkas_valid='', waktu_input_berkas_valid='0000-00-00 00:00:00' WHERE id_berkas_ujian_kelas='$id_berkas_ujian_kelas' AND nama_berkas='Nilai Hasil Ujian';");
	// 	return $hasil;
	// }
